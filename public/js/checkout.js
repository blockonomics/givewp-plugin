...
class Blockonomics {
    ...
    refresh_order() {
        ...
        self = this;
        jQuery.post( this.data.get_order_amount_url, data, function(response) {
            // var res = JSON.parse( response );
            self._update_order_params(response);
            self._set_refresh_loading(false);
        });
        // const data = new FormData();
        // data.append( 'action', 'foobar' );
        
        // fetch(this.data.get_order_amount_url, { method: 'POST', body: data })
        //     .then((res) => {
        //         if (!res.ok) {
        //             location.reload();
        //         } else {
